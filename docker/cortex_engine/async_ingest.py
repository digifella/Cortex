...
class AsyncIngestionEngine:
    ...
    def _process_file_sync(self, file_path: str) -> Dict:
        ...
        source_type = str(content_result.get("source_type", "document") or "document")
        combined_text = f"{os.path.basename(file_path)}\n{content_result.get('content', '')}".lower()
        ai_markers = ["ai generated report", "generated by ai", "chatgpt", "openai", "claude", "gemini", "perplexity"]
        is_ai_generated = any(marker in combined_text for marker in ai_markers)
        if is_ai_generated and source_type not in {"image", "image_qwen3vl", "image_fallback", "image_error", "image_skipped"}:
        ...
        flat_metadata = {
            ...
            "content_origin": "ai_generated" if source_type == "AI Generated Report" else "human_or_unknown",
