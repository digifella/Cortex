...
- Mixture of Experts (MoE) with multiple 70B models for superior analysis

Replaces: AI Assisted Research + Knowledge Synthesizer
Workflow: Input → Intent Classification → Parallel Execution → MoE Synthesis → Streaming Output
"""
...
class UniversalKnowledgeAssistant:
    ...
    async def _get_power_llm(
        self,
        task_type: TaskType = TaskType.RESEARCH,
        force_model: Optional[str] = None
    ) -> tuple[ModernOllamaLLM, str, str]:
        """Get or create power LLM for analysis/synthesis. Returns (llm, model_name, reasoning)."""
        # Model selection reasoning
        task_reasoning = {
            TaskType.RESEARCH: "Comprehensive research and analysis capabilities",
            TaskType.SYNTHESIS: "Strong pattern recognition and synthesis abilities",
            TaskType.IDEATION: "Creative thinking and novel idea generation",
            TaskType.ANALYSIS: "Balanced analysis with factual grounding",
        }

        if force_model:
            model_name = force_model
            reasoning = f"Manually selected by user"
            logger.info(f"Using manually selected model: {model_name}")
        elif self._power_llm is None:
            model_name = await self.model_manager.recommend_model(
                task_type,
                preference="best"
            )
            reasoning = f"Auto-selected for {task_type.value}: {task_reasoning.get(task_type, 'Best available model')}"
            logger.info(f"Using power model for {task_type}: {model_name}")
        else:
            # Return existing LLM and its model name
            return self._power_llm, self._power_llm.model, "Previously selected model (cached)"

        self._power_llm = ModernOllamaLLM(model=model_name)
        return self._power_llm, model_name, reasoning
...
Respond with just the category name (e.g., "RESEARCH"), nothing else."""
    ...
    async def synthesize_stream(
        ...
        Args:
            ...
            force_model: Optional model name to force use
        ...
        power_llm, model_name, model_reasoning = await self._get_power_llm(task_map[intent], force_model=force_model)

        # Count total results
        total_results = sum(len(results) for results in search_results.values())

        # Yield initial status with model info
        yield SynthesisChunk(
            content=f"Found {total_results} relevant sources. Analyzing...",
            chunk_type="text",
            metadata={"status": "analyzing", "model": model_name, "reasoning": model_reasoning}
        )
        ...
        logger.info(f"Streaming synthesis for {intent.value} query with temperature={temperature}")

        try:
            async for chunk in power_llm.astream_complete(prompt, temperature=temperature):
        ...
        for source_type, results in search_results.items():
            if results:
                for result in results[:5]:  # Top 5 sources
                    # Extract best available title and source info
                    ...
                    if not title or title == "Untitled Document":
                        ...
                        title = first_line if first_line else "Document"

                    # Try multiple fields for collection/source
                    collection = (
                        metadata.get("collection") or
                        metadata.get("source_collection") or
                        metadata.get("folder") or
                        metadata.get("category")
                    )

                    if collection:
                        source_text = f"- **{title}** (Collection: {collection})"
                    else:
                        # Show document ID or file path if available
                        doc_id = metadata.get("doc_id") or metadata.get("id")
                        if doc_id:
                            source_text = f"- **{title}** (ID: {doc_id[:12]}...)"
                        else:
                            source_text = f"- **{title}**"
                    ...
                    if result.relevance_score > 0:
                        source_text += f" (Relevance: {result.relevance_score:.2f})"
    ...
    async def moe_synthesize_stream(
        self,
        query: str,
        search_results: Dict[str, List[SearchResult]],
        intent: IntentType,
        ...
        """
        Mixture of Experts synthesis - runs multiple models and combines outputs.
        ...
        Yields:
            Synthesis chunks including expert outputs and final combination
        """
        logger.info(f"Starting MoE synthesis with multiple expert models")

        # Define expert models to use
        expert_models = [
        ...
        # Yield status
        yield SynthesisChunk(
            content="🎯 **MoE Mode:** Running multiple expert models...",
            chunk_type="text",
            metadata={
                "status": "moe_starting",
                "model": "MoE Meta-Synthesizer",
                "expert_models": expert_models
            }
        )

        # Build context from search results (same as regular synthesis)
        context_parts = []
        for source_type, results in search_results.items():
            if results:
                context_parts.append(f"\n## {source_type.replace('_', ' ').title()} Results\n")
        ...
        # Run expert models in parallel
        expert_outputs = []

        for i, model_name in enumerate(expert_models, 1):
            yield SynthesisChunk(
                content=f"\n\n### Expert {i}: {model_name}\n\n",
                chunk_type="text",
                metadata={"status": f"expert_{i}_running"}
            )

            try:
                # Create LLM for this expert with extended timeout for large models
                expert_llm = ModernOllamaLLM(model=model_name, request_timeout=600.0)  # 10 minutes
                logger.info(f"Expert {i} LLM created with timeout: {expert_llm.request_timeout}s")

                # Get complete response (non-streaming for simplicity in MoE)
                response = await expert_llm.acomplete(prompt, temperature=temperature)
                expert_output = response.text
                ...
                # Show expert output in collapsible section
                yield SynthesisChunk(
                    content=f"<details><summary>View Expert {i} Analysis ({model_name})</summary>\n\n{expert_output}\n\n</details>\n\n",
                    chunk_type="text",
                    metadata={"expert": i, "model": model_name}
                )

            except Exception as e:
                logger.error(f"Expert {i} ({model_name}) failed: {e}")
                yield SynthesisChunk(
                    content=f"⚠️ Expert {i} encountered an error\n\n",
                    chunk_type="text",
                    metadata={"error": str(e)}
                )

        # Meta-synthesis: Combine expert outputs
        if len(expert_outputs) >= 2:
            yield SynthesisChunk(
                content="\n\n---\n\n## 🎯 Combined Synthesis\n\n",
                chunk_type="text",
                metadata={"status": "meta_synthesis"}
            )

            # Build meta-synthesis prompt
            meta_prompt = f"""You are a meta-analyst expert at synthesizing multiple expert analyses into superior insights.

Below are {len(expert_outputs)} expert analyses of the same query. Your task is to create a SINGLE, SUPERIOR synthesis that:

1. **Takes the best insights** from each expert
2. **Resolves contradictions** by evaluating which perspective is more sound
3. **Adds unique value** beyond what either expert provided individually
4. **Maintains clear structure** with headers and bullet points
5. **Provides actionable takeaways** that combine the strengths of both
...
"""
            ...
            for i, expert in enumerate(expert_outputs, 1):
                meta_prompt += f"""
### Expert {i} Analysis ({expert['model']}):
...
"""
            ...
            meta_prompt += """
Now, synthesize these expert analyses into ONE cohesive, high-quality response that represents the best thinking from all experts:"""

            # Use first expert model as meta-synthesizer with extended timeout
            meta_llm = ModernOllamaLLM(model=expert_models[0], request_timeout=600.0)  # 10 minutes

            try:
                # Stream the meta-synthesis
                async for chunk in meta_llm.astream_complete(meta_prompt, temperature=temperature * 0.8):
                    yield SynthesisChunk(
                        content=chunk.text,
                        chunk_type="text"
                    )
                ...
                yield SynthesisChunk(
                    content=f"\n\n⚠️ Meta-synthesis encountered an error. Showing expert analyses above.\n\n",
                    chunk_type="text",
                    metadata={"error": str(e)}
                )

        # Yield sources at the end (same as regular synthesis)
        yield SynthesisChunk(
            content="\n\n## Sources\n",
            chunk_type="source"
        )
    ...
    def _build_synthesis_prompt(
        ...
        base_instruction = {
            IntentType.RESEARCH: """Provide a comprehensive research synthesis that:
- Identifies key themes and patterns across sources
- Highlights emerging trends and insights
- Connects related concepts and frameworks
- Provides actionable takeaways""",
            IntentType.SYNTHESIS: """Create an engaging synthesis that:
- Distills complex information into clear insights
- Identifies patterns and connections across sources
- Highlights contradictions or tensions in the knowledge
- Provides practical implications and recommendations
- Uses compelling examples and specific details""",
            IntentType.IDEATION: """Generate innovative ideas by:
- Combining concepts from different sources creatively
- Identifying gaps and opportunities
- Proposing novel solutions or approaches
- Providing concrete next steps for implementation""",
            IntentType.QUESTION: """Provide a direct, insightful answer that:
- Addresses the specific question clearly
- Supports claims with evidence from sources
- Acknowledges complexity or nuance where relevant
- Offers practical context and implications""",
            IntentType.EXPLORATION: """Explore the topic comprehensively by:
- Examining multiple perspectives and viewpoints
- Identifying key dimensions and frameworks
- Highlighting important debates or tensions
- Suggesting areas for deeper investigation"""
        }[intent]

        depth_instruction = {
            DepthLevel.QUICK: "Keep the response concise but impactful (2-3 focused paragraphs with bullet points).",
            DepthLevel.THOROUGH: "Provide a balanced, thorough analysis (4-6 well-developed paragraphs with clear structure).",
            DepthLevel.DEEP: "Provide an extensive, multi-faceted analysis with clear sections, subheadings, and comprehensive coverage."
        }[depth]

        prompt = f"""You are an expert knowledge analyst specializing in synthesizing complex information into clear, actionable insights.
...
## Your Task
Create a compelling, well-structured response that synthesizes the knowledge above to address the user's query.

**Requirements:**
- Use clear markdown formatting with headers (##, ###), bullet points, and emphasis
- Start with a brief, engaging introduction that frames the synthesis
- Organize insights into logical themes or sections
- Include specific examples, frameworks, or details from the sources
- End with actionable takeaways or recommendations
- Write in an engaging, professional tone - avoid generic corporate speak
- Do NOT cite sources inline - they will be listed separately
...
Begin your synthesis now:"""
    ...
    async def process_query(
        ...
        # Step 4: Stream synthesis (MoE or single model)
        if use_moe:
            # Use Mixture of Experts approach
            async for chunk in self.moe_synthesize_stream(
                user_input,
                search_results,
                intent,
                depth,
