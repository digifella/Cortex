#!/usr/bin/env python3
"""
Model Availability Checker (Docker copy)
...
from cortex_engine.config import VLM_MODEL, KB_LLM_MODEL, RESEARCH_LOCAL_MODEL
...
class ModelAvailabilityChecker:
    ...
    def _init_client(self):
        ...
        try:
            ok, _, url = check_ollama_service()
            if ok and url:
                self.client = ollama.Client(host=url)
            else:
                self.client = ollama.Client()  # fallback
    ...
    def format_status_message(self, check_results: Dict) -> str:
        ...
        if check_results.get("can_proceed"):
        ...
        if check_results.get("recommended_actions"):
