...
from cortex_engine.url_ingestor import URLIngestResult, URLIngestor, normalize_url_list
...
def render_url_ingestor_ui(*, standalone: bool = False) -> None:
    if standalone:
        ...
        st.markdown(
            "Paste URLs or upload a source list to discover open-access PDFs and optionally convert them to Markdown."
        )
    ...
    with st.container(border=True):
        ...
        convert_to_md = c1.checkbox("Convert downloaded PDFs to Markdown", value=True, key="url_ingestor_convert_to_md")
        use_vision_for_md = c2.checkbox("Use vision during PDF->MD conversion", value=False, key="url_ingestor_use_vision")
        capture_web_md_on_no_pdf = c3.checkbox(
            "Capture web page as Markdown when PDF unavailable",
            value=True,
            key="url_ingestor_capture_web",
        )
        timeout_seconds = st.number_input(
            ...
            value=25,
            step=5,
            key="url_ingestor_timeout_seconds",
        )
        ...
        run_btn = st.button(
            ...
            disabled=(db_root is None),
            key="url_ingestor_run_btn",
        )
    ...
    if results:
        with st.container(border=True):
            ...
            if zip_bytes:
                c3.download_button(
                    "Download PDFs + Markdown + Reports (ZIP)",
                    zip_bytes,
                    file_name="url_ingest_bundle.zip",
