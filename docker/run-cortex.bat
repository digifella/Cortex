...
if defined ENV_AI_DB_PATH (
    ...
    REM Clear old entries from .env - will be re-added with user confirmation


    findstr /V /B "WINDOWS_AI_DATABASE_PATH= AI_DATABASE_PATH= WINDOWS_KNOWLEDGE_SOURCE_PATH= KNOWLEDGE_SOURCE_PATH=" .env > .env.tmp
...
REM ALWAYS prompt for AI Database Path (with default if exists)
...
REM ALWAYS prompt for Knowledge Source Path (with default if exists)
