...
## Source-Type Resolution Guardrails
- Apply a deterministic final pass over normalized markdown/title/publisher after any LLM metadata output.
- Prefer explicit human-source markers over weak consulting hints:
  - Institutional/government markers (e.g., ADHA/AIDH/department/ministry/WHO/OECD/World Bank/UN) should not be overridden by substring matches such as `ey`.
- AI-generated detection should include both marker and style signals:
  - markers: `chatgpt|openai|claude|gemini|perplexity|generated by ai|deep report|deep research`
  - style cue: unusually frequent em-dashes in the body when strong human-source markers are absent.
- If human-source evidence is weak and AI markers/style are present, classify `source_type = "AI Generated Report"`.
