        ...
        if slice_result:
            ...
            if provider == "gemini":
                ...
                with retry_c1:
                    auto_retry_sliced_quota = st.checkbox(
                        ...
                        value=bool(st.session_state.get("included_study_sliced_auto_retry_quota", True)),
                        key="included_study_sliced_auto_retry_quota",
                    )
                with retry_c2:
                    sliced_retry_wait_cap = int(
                        st.number_input(
                            "Max retry wait (s)",
                            min_value=10,
                            max_value=180,
            ...
            if selected and st.button(
                ...
                try:
                    ...
                    def _slice_progress(message: str) -> None:
                        progress_lines.append(f"{time.strftime('%H:%M:%S')} {message}")
                        progress_box.text_area(
                            "Sliced extraction log",
...
def main():
    ...
    tab_textifier, tab_included_study, tab_study_miner, tab_research, tab_url_ingest, tab_pdfimg, tab_anonymizer = st.tabs(
        ["Textifier", "Included Study Extractor", "Study Miner", "Research Resolver", "URL PDF Ingestor", "PDF Image Extractor", "Anonymizer"]
    )
